...
public class GateSlot : MonoBehaviour
{
    ...
    private void Awake() {
        ...
        slot = this.transform.parent.parent.parent.parent.Find("puzzle.gate.machine/puzzle.unlock/wire.slot.withDoor").GetComponent<DoorSlot>();     
    }
    private void hitObject(){}
    private void OnTriggerEnter(Collider other) {
        if(other.TryGetComponent<Gate>(out Gate gateInput)){
            gateInput.isInSlot += gateInput.getVoltageFromHit;
            Debug.Log(!this.slot.triggerSlots.Contains(this));
